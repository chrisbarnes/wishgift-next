...
describe('CheckboxToggle', () => {
  ...
  it('renders checkbox input', () => {
    render(<CheckboxToggle label="Toggle" />);
    const checkbox = screen.getByRole('checkbox');
    expect(checkbox).toBeInTheDocument();
  });

  it('sets default checked state', () => {
    render(<CheckboxToggle label="Toggle" isChecked={true} />);
    const checkbox = screen.getByRole('checkbox');
    expect(checkbox).toBeChecked();
  });

  it('sets default unchecked state', () => {
    render(<CheckboxToggle label="Toggle" isChecked={false} />);
    const checkbox = screen.getByRole('checkbox');
  ...
  it('applies correct CSS classes to label', () => {
    ...
    expect(label).toHaveClass('relative', 'flex', 'justify-between', 'items-center');
  });
});
